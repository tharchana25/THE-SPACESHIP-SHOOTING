...
#define PI 3.14159
#define GAME_SCREEN 0			// to identify background color
...
int alienLife=100;
int GameLvl= 1;
float mouseX ,mouseY ;				//Cursor(mouse) coordinates;
...
float xlifeBarStart = 1200;				//life bar starting coodinate
...
{
	//if(key=='u' && key=='r' ){xOne+=0.5;yOne+=0.5;}
	if(key == 'r') xOne+=SPACESHIP_SPEED; 
	if(key == 'a') xOne-=SPACESHIP_SPEED; 
	if(key == 'u') {yOne+=SPACESHIP_SPEED;}
	if(key == 'd') {yOne-=SPACESHIP_SPEED;}
	if(key == 'r' || key == 'a' || key == 'u' || key == 'd')
